...
import {
  ...
  querySnippetCategoryList,
  removeSnippetCategory,
  updateSnippetCategory,
} from '@/services/ant-design-pro/snippetCategory'
...
const SnippetCategoryList = () => {
  ...
  const handleReload = () => {
    setLoading(true)
    querySnippetCategoryList().then(({ data }) => {
      setTimeout(() => {
        setLoading(false)
  ...
  const handleRemove = (entity: API.Category) => () => {
    Modal.confirm({
      ...
      content: '删除后不可恢复',
      onOk() {
        removeSnippetCategory(entity.id!).then(() => {
  ...
  const confirmUpdate = (values: CategoryActionRequest) => {
    ...
    // 有ID 表示更新
    return updateSnippetCategory(temp?.id!, values).then(() => {
      reset('更新成功')
    })
  }
  ...
  const confirmCreate = (values: CategoryActionRequest) => {
    ...
    return createSnippetCategory(values).then(() => {
      reset('创建成功')
    })
  }
...
export default SnippetCategoryList
